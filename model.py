...
class ModelArgs:
    ...
    uniform_wqk: bool = False
    identity_wqk: bool = False
...
class SelfAttention(nn.Module):
    def __init__(self,
        ...
        freeze_wqk=args.freeze_wqk
        uniform_wqk=args.uniform_wqk
        identity_wqk=args.identity_wqk
        ...
        self.attention = Attention(
                ...
                freeze_wv=freeze_wv,
                freeze_wo=freeze_wo,
                freeze_wqk=freeze_wqk,
                uniform_wqk=uniform_wqk,
                identity_wqk=identity_wqk,
                no_wo=no_wo,
                no_wv=no_wv)
    ...
    def forward(self,
        ...
        h, scores = self.attention(h, mask)

        return self.output(h)

    def get_attention_score(self,tokens: torch.Tensor):
        B, N = tokens.shape
        h = self.tok_embeddings(tokens)
    ...
    def get_attention_activation(self,tokens: torch.Tensor):
        B, N = tokens.shape
        h = self.tok_embeddings(tokens)
        ...
        h = self.attention.get_attention_activation(h, mask)
...
class Attention(nn.Module):
    def __init__(self,
        ...
        if uniform_wqk:
            self.wq.weight = nn.Parameter(torch.zeros_like(self.wq.weight))
            self.wq.weight.requires_grad_(False)
        ...
        if uniform_wqk:
            self.wk.weight = nn.Parameter(torch.zeros_like(self.wk.weight))
            self.wk.weight.requires_grad_(False)
    ...
    def get_attention_activation(self,
                x: torch.Tensor,
                mask: torch.Tensor):
        bs, slen, _ = x.shape
        assert mask is not None
